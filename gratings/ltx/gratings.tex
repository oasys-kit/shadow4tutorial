  ...
  \begin{figure}[H]
    ...
    \begin{tikzpicture}
      \draw[fill=plbg, draw=plframe, line width=1pt, dashed]
            (0,0) rectangle (#1, #2);
      % diagonal cross
      \draw[plframe, line width=0.4pt] (0,0) -- (#1,#2);
      \draw[plframe, line width=0.4pt] (#1,0) -- (0,#2);
      % label tag
      \node[anchor=north west, font=\small\bfseries\color{plframe},
            inner sep=4pt] at (0,#2) {#3};
      % description
...
\lhead{\small\itshape Reininger \& Sanchez del Rio}
\cfoot{\thepage}
...
\begin{titlepage}
  ...
  {\normalsize Based on lectures by \textbf{Ruben Reininger}\\
   ...
   DESY Hamburg, May 2025\\
   ESRF Grenoble, May 2026}\\[4em]
  {\normalsize\today}
  \vfill
\end{titlepage}
...
(across the groove direction) and the meridional coordinate $h$ (along the grooves):
...
\begin{eqnarray}
\frac{1}{d} = k = \frac{\partial n}{\partial w} &=& \frac{1 + 2b_2 w + 3b_3 w^2 + \ldots}{d_0} \\[10pt]
F_{00} &=& r + r' \\[10pt]
F_{10} &=& m\frac{\lambda}{d_0} - (\sin\alpha + \sin\beta) \\[10pt]
F_{20} &=& \frac{1}{2}\left(\frac{\cos^2\alpha}{r} - \frac{\cos\alpha}{R} + \frac{\cos^2\beta}{r'} - \frac{\cos\beta}{R}\right) - \frac{m\lambda\, b_2}{d_0} \\[10pt]
F_{02} &=& \frac{1}{r} - \frac{\cos\alpha}{\rho} + \frac{1}{r'} - \frac{\cos\beta}{\rho} \\[10pt]
F_{12} &=&  \frac{1}{2}\left(\frac{\sin\alpha}{r}\left(\frac{1}{r} - \frac{\cos\alpha}{\rho}\right) + \frac{\sin\beta}{r'}\left(\frac{1}{r'} - \frac{\cos\beta}{\rho}\right)\right)
\\[10pt]
F_{30} &=& \frac{1}{2}\left(\frac{\sin\alpha}{r}\left(\frac{\cos^2\alpha}{r} - \frac{\cos\alpha}{R}\right) + \frac{\sin\beta}{r'}\left(\frac{\cos^2\beta}{r'} - \frac{\cos\beta}{R}\right)\right) - \frac{m\lambda\, b_3}{d_0}
\end{eqnarray}


Note that for a grating with constant ruling $b_2=b_3=0$.
...
\caption{
  Geometry of a toroidal grating showing source point $S$, grating vertex, image
  point $S'$, sagittal coordinate $h$, meridional coordinate $w$,
  angles of incidence $\alpha$ and diffraction $\beta$, and radii of curvature
  $R$ (meridional) and $\rho$ (sagittal).}
\label{fig:schematic}
  \begin{center}
  \includegraphics[width=0.69\textwidth]{figures/schematic.png}   
  \end{center}
\end{figure}
...
$F_{20}$ & Meridional Focus   & Shifts focus along optical axis \\
$F_{02}$ & Sagittal Focus     & Sagittal focus mismatch \\
$F_{30}$ & Primary coma           & Asymmetric blur focal plane \\
\bottomrule
\end{tabular}
\end{center}
...
$F_{20} = 0$: meridional focus at the exit slit position:
\begin{equation}
    \frac{\cos^2\alpha}{r} + \frac{\cos^2\beta}{r'} = \frac{\cos\alpha + \cos\beta}{R} + \frac{2m\lambda\, b_2}{d_0}
\end{equation}

$F_{02} = 0$: Sagittal focus at the exit slit position:

\begin{equation}
\frac{1}{r} + \frac{1}{r'} = \frac{\cos\alpha + \cos\beta}{\rho}
\end{equation}

$F_{30} = 0$: No Primary Coma:

\begin{equation}
    \frac{\sin\alpha}{r}\left(\frac{\cos^2\alpha}{r} - \frac{\cos\alpha}{R}\right) + \frac{\sin\beta}{r'}\left(\frac{\cos^2\beta}{r'} - \frac{\cos\beta}{R}\right) - \frac{2m\lambda\, b_3}{d_0} = 0
\end{equation}

Note that for the constant ruling grating $b_3=0$. On the ``Rowland circle" ($r = R \cos\alpha$, $r' = R \cos\beta)$, both terms vanish individually, so coma is automatically zero (a good key property of the Rowland circle mounting).
...
\begin{equation}
  ...
  \label{eq:grating}
\end{equation}
where $d$ is the groove spacing, $\al$ is the angle of incidence,
and $\be$ is the diffraction angle. Positive angles are counterclockwise (CCW). In SHADOW notation
$m = -1$  corresponds to the first order ``internal reflection'' ($|\be| > |\al|$).
...
\textbf{Parameters:} $r = 30$\,m, $r' \approx 9.9$\,m, $\al = 87.3^\circ$, $\be = 89.1^\circ$, $R =636$\,m, $\rho =9.9$\,m,
diffraction order $m = -1$.

\medskip
\textbf{Results:}
\begin{itemize}
  \item Predicted vertical and horizontal image sizes (sanity check):

  
  $\Delta_z = 2.35\,\sigma_z\,(\cos\al/\cos\be)\,(r'/r) \approx 36\,\mu$m.
  
  $\Delta_x \approx 217\,\mu$m (at the position of sagittal focus!).
  \item Achieved resolving power: $\RP \approx 18\,000$ at 1000\,eV with 55\,meV FWHM
        energy resolution.
  \item Ray-tracing gives $\Delta_z = 35~\mu$m, $\Delta_x = 2030~\mu$m.
\end{itemize}

\medskip
\textbf{Slit scan:} Using source in the photon energy interval 999.8 $< E < $ 1000.2~eV, the resolution is obtained from the FWHM of the intensity-energy histogram. Using a slit of 3$\mu$m, the resulting $\Delta E=$0.552~eV (RP = 18116).  It is verified that roughly constant resolution is obtained slit openings
$\lesssim 20\,\mu$m (\emph{source limited}).
For larger openings ($\gtrsim 50\,\mu$m) the resolution is
...
\begin{figure}[H]
\caption{
  SHADOW ray-tracing results for SGM at 1000\,eV, including spot dimensions at the exit slit plane,
  energy resolution histogram, and energy resolution vs.\ slit opening, showing source-limited and slit-limited
  regimes with the transition near $20-50\,\mu$m.}
\label{fig:SGM1000}
  \begin{center}
  \includegraphics[width=0.99\textwidth]{figures/SGM1000.png}   
  \end{center}
\end{figure}
...
\textbf{Parameters:} $\al = 87.63^\circ$, $\be = -88.77^\circ$.  Source: $\Sigma_x = 280\,\mu$m,
$\Sigma_z = 13.7\,\mu$m, $\Sigma_x' = 18.4\,\mu$rad, $\Sigma_z' = 14.2\,\mu$rad.
...
if the exit slit remains at the 1000\,eV position (the vertical size is 55.6\,$\mu$m FWHM).  Moving the exit slit to
...
The \textbf{grating magnification} factor is $c^{-1}$, with $c$ defined using the Rowland circle condition:
\begin{equation}
  c = \frac{r'}{r} = \frac{R \cos\be}{R \cos\al} = \frac{\cos\be}{\cos\al},
  \label{eq:c_factor}
\end{equation}
and the grating equation becomes:
\begin{equation}
  \left( \frac{m\la}{d} - \sin\be \right)^2 = 1 - \frac{1-\sin^2\be}{c^2}.
  \label{eq:PGM_grating}
\end{equation}

The parameter $c$ controls the angular compression/expansion between the collimated beam
and the diffracted beam, and thus the grating magnification $c^{-1}$ can be
tuned by rotating the plane mirror.
...
\textbf{Layout} (distances from source): Toroidal mirror at 28\,m $\to$ plane mirror +
plane grating at 2\,m downstream from the Toroidal mirror. Cylindrical mirror at 1\,m downstream from the grating. Image plane 9\,m further downstream. 
...
\medskip
\textbf{Parameters at 1000\,eV, $c = 2$:} $\al = 88.5^\circ$, $\be = 87.1^\circ$; plane mirror at
$87.8^\circ$ incidence.
...
\begin{itemize}
  ...
  \item Image size at slit: $\sim 5.8\,\mu$m (theoretical), $\sim 6.8\,\mu$m (ray-tracing, evidencing coma).
  \item \textbf{Slope errors:} It is shown that the slope errors on the toroidal mirror broaden the image significantly at the image plane (passing from 6.8~$\mu$m to 18.4~$\mu$m).
  Therefore, the slope errors on the upstream toroid are the dominant resolution-limiting factor in this configuration.
\end{itemize}
\end{exbox}
...
\caption{
  Effect of slope errors on the toroidal mirror in the vertical PGM.
  The image on the slit plane with realistic r.m.s. slope errors is  $\sim 18.4\,\mu$m (ideal, no slope errors: $\sim 5.8\,\mu$m).
  This demonstrates the degradation of the resolution due to the slope errors that affect the collimation in the diffraction plane.}
\label{fig:PGM}
  \begin{center}
  \includegraphics[width=0.99\textwidth]{figures/PGMslope.png}   
  \end{center}
\end{figure}
...
The \textbf{Follath PGM} (R.\ Follath and F.\ Senf, NIM A\,390, 388, 1997) uses the toroidal mirror, such that the 
\textbf{horizontal} sagittal focusing is in the grating vertical (diffraction) plane. The key advantage is
that, in the sagittal plane, slope errors on the toroidal mirror are much less critical
because the sagittal focusing is less sensitive to meridional slope errors. This is expressed by the forgiveness factor\footnote{In the meridional plane, the slope errors $\sigma$ 
increase the focal size by a factor $2q\sigma$. In the sagittal 
direction, the same slope error increases the size by a factor 
$2q\sigma\sin\theta$, where $\theta$ is the grazing angle of 
incidence. Since $\theta \ll 1$ for X-ray mirrors, the sagittal 
contribution is reduced by the \textit{forgiveness factor} 
$\sin\theta \ll 1$.}
...
\texttt{05\_CollimatedPGM1000eVc5.ows}), $\al = 89.5^\circ$, $\be = 87.4^\circ$.  The
...
\begin{figure}[H]
\caption{ Left: Follath horizontal PGM ($c = 2$, 1000\,eV). Observe: no coma, practically no effect of slope error. Right:  Follath horizontal PGM ($c = 5$, 1000\,eV). Observe: Astigmatic coma (term $F_{12}$), small effect of slope error. }
\label{fig:Follath}
  \begin{center}
  \includegraphics[width=0.49\textwidth]{figures/follathc2.png}
  \includegraphics[width=0.49\textwidth]{figures/follathc5.png} 
  \end{center}
\end{figure}
...
\begin{equation}
  k = \frac{1 + 2 b_2\,w + 3 b_3\,w^2 + \cdots}{d_0},
  \label{eq:VLS}
\end{equation}
with $1/d_0$ the central line density.  The coefficients $b_2$ and $b_3$
are chosen to:

\begin{itemize}
  \item Choose the grating magnification $c^{-1}=(\cos\be/\cos\al)^{-1}$.
  \item Choose $\lambda_0$ for which $b_2$ will be zero. 
  \item Solve the grating equation \ref{eq:grating} for $\be_0$.
  \item $b_2$: Solve the defocus equation $F_{20} = 0$. 
  ...
  \item $b_3$: Cancel coma $F_{30} = 0$ at one selected wavelength.
  % ; coma is then acceptably small over the full energy range.
\end{itemize}
...
\begin{exbox}
\textbf{File:} \texttt{06\_FVLSPGM1000eVc2M2WithWithout.ows
}\\[4pt]
\textbf{Distances:} Source $\to$ heat absorber (placed artificially at 0~m) $\to$ plane mirror (29.7\,m) $\to$ VLS
grating (+0.3\,m) $\to$ slit plane (+10\,m) .\\
Grating designed for $c=2$ at 600~eV. \\
Parameters at 1000\,eV: $\al = 88.53^\circ$, $\be = 87.05^\circ$ (same $c = 2$ condition).
...
\[
  \al:\; 88.52843^\circ \to 88.53789^\circ,\quad
  \be:\; 87.05389^\circ \to 87.05860^\circ.
\]
...
\medskip
\textbf{Reference:} R.\ Reininger (2010).
\end{exbox}
...
\label{fig:VLS}
  \begin{center}
  \includegraphics[width=0.99\textwidth]{figures/VLS.png}   
  \end{center}
\end{figure}
...
% =======================================================================
A unified theoretical framework --- the optical path
length expansion of Eq.~\eqref{eq:OPL} and Fermat's principle --- underpins all
...
\begin{enumerate}[leftmargin=*, label=\arabic*.]
  ...
  \item \textbf{The collimated PGM} decouples focusing from dispersion.  The
        ...
        errors.  The \emph{Follath} layout eliminates coma and is far more
...
$R$                    & Grating radius of curvature (Meridional) \\
$\rho$                    & Grating radius of curvature (Sagittal) \\
$c$                    & Inverse of Magnification factor ($c= \cos\be/\cos\al$) \\
$\RP$                  & Resolving power ($RP= \la/\Delta\la=-E/\Delta E$) \\
$\Sigma_x, \Sigma_z$             & Source size (horizontal, vertical) RMS \\
$\Sigma_x', \Sigma_z'$           & Source divergence (horizontal, vertical) RMS \\
$b_2, b_3$             & VLS grating coefficients (line density gradient, curvature) \\
$1/d_0$             & Central groove density of VLS grating \\
\bottomrule
\end{tabular}
...
\begin{enumerate}
  ...
  \item R.\ Reininger,
        \textit{Correcting Heat Load Induced Deformations on a XUV Beamline},
        Proc. of ACTOP2008, Second Workshop on X-ray and XUV Optics, Trieste (Italy) (2008).

\href{https://www.elettra.eu/actop08/uploads/Main/a29.pdf}{link here}
        % https://www.elettra.eu/actop08/uploads/Main/a29.pdf
\end{enumerate}
