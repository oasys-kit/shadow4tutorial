...
Note that the columns 4,5,6 are the direction cosines (the components of the normalized direction vector). In the paraxial approximation, when the beam direction if nearly in the $y$ axis, the direction vector separates from the $y$ axis an angle $\theta_x$ ($\theta_x$) along the $x$ ($z$) axis. In this case, $\theta_x \approx v_x$, $\theta_z \approx v_z$ (see Table).

\begin{table}[h]
\centering
\begin{tabular}{ccc}
\hline
 & \textbf{Exact} & \textbf{Paraxial} \\
\hline
$a$ & angle to $\mathbf{e}_x$ & $a = 90^\circ - \theta_x$ \\
$b$ & angle to $\mathbf{e}_y$ & $b \approx 0^\circ$ \\
$c$ & angle to $\mathbf{e}_z$ & $c = 90^\circ - \theta_z$ \\
$\alpha=v_x$ & $\cos a$ & $\alpha \approx \theta_x$ \\
$\beta=v_y$  & $\cos b$ & $\beta \approx 1$ \\
$\gamma=v_z$ & $\cos c$ & $\gamma \approx \theta_z$ \\
\text{identity} & $\alpha^2 + \beta^2 + \gamma^2 = 1$ & $\theta_x^2 + 1 + \theta_z^2 \approx 1$ \\
\hline
\end{tabular}
\caption{Direction cosines under the paraxial approximation (propagation close to $\mathbf{e}_y$)}
\end{table}
